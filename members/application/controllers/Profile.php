	...
	public function _remap($username=null)
	 ...
	 { 	
		 	...
		 	if($user_detail!=false){
			 	...
			 	$privacy_data = $this->profile_model->getPrivacy($user_id);
	       ...
	       $ref_row = array('wgp_table_id', 'wgp_user_id','phone','email',
		    ...
		    $profile_pic_status = $this->photo_model->checkProfilePic($user_id);
			 ...
			 $per_psyhosocial = $this->getPsyhosocialDetail($user_id);
		        ...
		        $id_arry = array('friend_id'=>$friend_id,'user_id'=>$user_id);
		        $noti_data =  $this->notification_model->checkProfileSearchByUser($id_arry);
				...
				$ip = $_SERVER['REMOTE_ADDR'];			
		         ...
		         $today = date('Y-m-d');
		        if($noti_data){
			        foreach ($noti_data as $row) {

			      
			            $notification_date = date('Y-m-d',strtotime($row->notification_date));	
			            if( ($notification_date!=$today) && ($row->ip != $ip)){

			            	  //set notification data for ProfileView
				    	      $this->notification_model->setNotiProfileView($user_id,$friend_id,$ip);
				    	      //end notification ProfileView
			            }
			        }
			    }else{
			    	   

			    	  $today_noti_status =  $this->notification_model->checkProfileVisitedByIpToday($ip);
			    	  

			    	  if(!$today_noti_status){
		      ...
		      $data = array('detail' => $detail,'dp_url'=>$dp_url,'user_id'=>$user_id,
		      				'school'=>$school,'seeking'=>$seeking,
			  				 ...
			  				 'teaminfo'=>$teaminfo,'level'=>$level_data,'competition'=>$competition,
					   	  	 ...
					   	  	 'per_technical'=>$per_technical,'per_tachtical'=>$per_tachtical,
			   	  	 		 ...
			   	  	 		 'privacy_data'=>$privacy_data,'record'=>$record_data
			   	  	 );
